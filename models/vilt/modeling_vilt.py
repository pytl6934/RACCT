...
class MoEFFN(nn.Module):
    def __init__(self, hidden_size, intermediate_size, num_experts=4, top_k=2, dropout=0.1):
        super().__init__()
        self.num_experts = num_experts
        self.top_k = top_k
        self.hidden_size = hidden_size

        # 路由器：决定每个 token 应该送到哪些专家
        self.router = nn.Sequential(
            nn.Linear(hidden_size, num_experts),
            nn.Softmax(dim=-1)
        )

        # 专家们，每个是一个 FFN 层（Linear → GELU → Linear）
    ...
    def forward(self, x):
        ...
        # 1. 获得路由权重
        route_weight = self.router(x)   # [B, L, E]

        if self.top_k < self.num_experts:
            # Top-k masking
            topk_val, topk_idx = route_weight.topk(self.top_k, dim=-1)
            mask = torch.zeros_like(route_weight)
            mask.scatter_(-1, topk_idx, 1.0)
            route_weight = route_weight * mask
            route_weight = route_weight / (route_weight.sum(-1, keepdim=True) + 1e-9)

        # 2. 各专家并行计算
        expert_outputs = []
        for expert in self.experts:
            expert_outputs.append(expert(x))  # 每个 [B, L, D]
        expert_outputs = torch.stack(expert_outputs, dim=-2)  # [B, L, E, D]

        # 3. 路由加权
        route_weight = route_weight.unsqueeze(-1)  # [B, L, E, 1]
        output = (expert_outputs * route_weight).sum(dim=-2)  # [B, L, D]
...
class ExpertRouter(nn.Module):
    ...
    def forward(self, x):
        # x: [B, L, D] → return [B, L, num_experts]
...
class ImageSelfAttention(nn.Module):
    ...
    def forward(self, image_tokens):
        return self.attn(image_tokens, image_tokens, image_tokens)[0]
...
class TextSelfAttention(nn.Module):
    ...
    def forward(self, text_tokens):
        return self.attn(text_tokens, text_tokens, text_tokens)[0]


class CrossAttention(nn.Module):
    def __init__(self, hidden_size, nhead):
        super().__init__()
        self.cross_text = nn.MultiheadAttention(hidden_size, nhead, batch_first=True)
        self.cross_image = nn.MultiheadAttention(hidden_size, nhead, batch_first=True)

    def forward(self, text_tokens, image_tokens):
        # text queries image
        text_out = self.cross_text(text_tokens, image_tokens, image_tokens)[0]
        # image queries text
        image_out = self.cross_image(image_tokens, text_tokens, text_tokens)[0]
        return torch.cat([text_out, image_out], dim=1)  # 返回 [B, L, D]


class ModalityAwareMoEBlock(nn.Module):
    """
    封装 text/image 切分、Router、3 个专家和 top-k 加权融合的逻辑
    """
    def __init__(self,
                 hidden_size: int,
        ...
        # Router: [B, L, num_experts]
        self.router = nn.Sequential(
            nn.Linear(hidden_size, num_experts),
            nn.Softmax(dim=-1)
        )

        # Expert 0: image-only self-attn
        self.image_expert = nn.MultiheadAttention(hidden_size, num_attention_heads,
                                                  batch_first=True)
        # Expert 1: text-only self-attn
        self.text_expert  = nn.MultiheadAttention(hidden_size, num_attention_heads,
                                                  batch_first=True)
        # Expert 2: cross-attn
        self.cross_text2img = nn.MultiheadAttention(hidden_size, num_attention_heads,
                                                    batch_first=True)
        self.cross_img2text = nn.MultiheadAttention(hidden_size, num_attention_heads,
                                                    batch_first=True)
    ...
    def forward(self,
        ...
        """
        layer_output: [B, L=T+V, D]
        text_mask:    [B, T]
        image_mask:   [B, V]
        """
        B, L, D = layer_output.shape
        T, V = self.text_len, self.image_len

        # 1. 切分
        text_tokens  = layer_output[:, :T, :]   # [B, T, D]
        image_tokens = layer_output[:, T:T+V, :]   # [B, V, D]

        # 2. Router 得到 [B, L, E]
        route_weights = self.router(layer_output)  # already softmax

        # 3. 专家计算
        # Expert0: image-only
        img_out, _ = self.image_expert(image_tokens,
                                       context,
                                       context,)
        expert0 = torch.cat([
            text_tokens,  # [B, T, D]
        ...
        # Expert1: text-only
        txt_out, _ = self.text_expert(text_tokens,
                                      context,
                                      context,
        ...
        # Expert2: cross-attn
        t2i, _ = self.cross_text2img(text_tokens,
                                     image_tokens,
                                     image_tokens,
              )
        i2t, _ = self.cross_img2text(image_tokens,
                                     text_tokens,
                                     text_tokens,
                                     key_padding_mask=text_mask)
        expert2 = torch.cat([t2i, i2t], dim=1)  # [B, L, D]

        # 4. top-k 路由（可选）
        # if self.top_k < route_weights.size(-1):
        ...
        #     route_weights = route_weights / (route_weights.sum(-1, keepdim=True) + 1e-9)

        # 5. 加权融合
        experts = torch.stack([expert0, expert1, expert2], dim=-2)  # [B, L, 3, D]
        rw = route_weights.unsqueeze(-1)                            # [B, L, 3, 1]
        moe_output = (experts * rw).sum(dim=-2)                      # [B, L, D]
...
class ViltLayer(nn.Module):
    ...
    def __init__(self, config):
        ...
        # self.moe_ffn = ModalityAwareMoEBlock(
    ...
    def forward(self, hidden_states, context, txt_mask, attention_mask=None, head_mask=None, output_attentions=False):
        # originhidden = hidden_states
        # moeout = self.moe_ffn(hidden_states, context, txt_mask)
        ...
        # layer_output = layer_output + moeout
