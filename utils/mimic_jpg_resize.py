...
def main():
    in_files = "./data/MIMICCXR/files"

    # resized_dir = os.path.join(os.path.dirname(args.jpgs), '..', 'mimic-cxr-resized')
    resized_dir = "./data/MIMICCXR/mimic-cxr-resized"
    out_files = "./data/MIMICCXR/mimic-cxr-resized/files"
    resize = Resize(256)
    ...
    print('Total {0} images Resized'.format(count_resized + count_skipped))
    print('Total {0} images skipped'.format(count_skipped))


def parse_args():
    parser = argparse.ArgumentParser(description='Resize MIMIC-CXR-JPG jpgs to 256 pixels pngs')
    parser.add_argument('jpgs', type=str, help='A path to MIMIC-CXR-JPG', default="./data/MIMICCXR")
