...
class ClassificationTrainer:
    ...
    def load_model(self):
        if self.isRAG == "RAG":
            ...
            file_path = f'./ckpts/Server/{self.args.prefix}.pth'
    ...
    def save_best(self, comms, logger):
        ckpt_path =f"./ckpts/Final/{self.args.prefix}.pth"
    ...
    def save_bestRAG(self, comms, logger):
        ...
        print(f"test round: {comms}, save server best model ")
        logger.info(f"test round: {comms}, save server best model, {self.args.prefix}")
        torch.save(self.model.state_dict(), f"./ckpts/Server/{self.args.prefix}.pth")

    def load_bestRAG(self):
        ckpt_path =f"./ckpts/Server/{self.args.prefix}.pth"
        checkpoint = torch.load(ckpt_path, map_location="cpu")
        self.model.load_state_dict(checkpoint)
