...
cntlr = 0
if cntlr == 0:
    print(f"---------------inilr {init_lr},  minlr {min_lr} --------------")
    cntlr+=1
...
class ClassificationTrainerRAG1():
    def __init__(self, args, client_id, config_path, modality, logger, dataname):
        ...
        self.perauc = 0.0
        self.loss = 0.0
        self.logger.info(f"---------------inilr {init_lr},  minlr {min_lr} --------------")

    def setup_rag(self):
        if self.dataname == "MIMIC":
            partition_path = f'./partitions/mimic-2.pkl'
            with open(partition_path, "rb") as f:
                d = pickle.load(f)

            train_set = MimicMultiModal(self.config.dataset.view, "train")
            train_idx = d[self.client_id]["train"] 
            test_idx = d[self.client_id]["test"] 
        ...
        if self.dataname == "chexpert":
            partition_path = "./partitions/chexpert2.pkl"
            with open(partition_path, "rb") as f:
                d = pickle.load(f)
    ...
    def generate_RAG_mapping(self, global_vec, global_labels, model, comms):
        ...
        torch.save(self.mappings1, f"./CARckpts/gptf/{self.dataname}_maptest_{self.client_id}.pth")
    ...
    def retrive_data1(self, local_idxs, global_idxs, mod):
        ...
        if self.round == 0:
            if mod == "train":
                ...
                global_idx = operator.itemgetter(*map_idx)(global_idxs)
                self.rdatatest[self.cnt] = global_idx                
    ...
    def run(self, comms, img_vec, txt_vec, labels, idxs, gmodel):
        ...
        if comms == 0:
            if not os.path.exists(f"./CARckpts/gptf/{self.dataname}_rdataidx_fx.pth"):
                torch.save(self.rdata, f"./CARckpts/gptf/{self.dataname}_rdataidx_fx.pth")
                print(f" save ./CARckpts/gptf/{self.dataname}_rdataidx_fx.pth")
            if not os.path.exists(f"./CARckpts/gptf/{self.dataname}_testrdataidx_fx.pth"):
                torch.save(self.rdatatest, f"./CARckpts/gptf/{self.dataname}_testrdataidx_fx.pth")
                print(f" save ./CARckpts/gptf/{self.dataname}_rdataidx_fx.pth")
    ...
    def traineta(self, global_idxs, model):
        model.eval()
        model.cuda()
            ...
            with tqdm(self.test_loader, unit="batch", desc="Testing") as tepoch:
                for data in tepoch:
                    ...
                    if self.modality != "mm":
                        ...
                        rframes = rd["img"].to(frames.dtype).cuda()
                        rtext = rd['txt']
                        rl = rd["rl"].cuda()
                    else:
                        rframes = 'a'
                    ...
                    evaluator.update(output["logits"].sigmoid(), label.long())
                    self.cnt += 1

        metrics = evaluator.compute()
        print( f" round: {self.round},test AUC(Aggregrated): {metrics['AUC'].item():.5f}, test ACC {metrics['ACC'].item():.5f} ")
        self.logger.info(f"round: {self.round} ,client data:{self.dataname}, Test AUC(Aggregrated): {metrics['AUC'].item():.5f}, test ACC {metrics['ACC'].item():.5f},  macroF1 {metrics['MultilabelF1Score'].item():.5f}")
