...
class FedAvgInRAG:
    def __init__(self, args, logger, wandb):
        ...
        self.warmup = 30
        self.mappingsval = {}
        self.mappingstest = {}# torch.load(f"./CARckpts/Server/mimictestMap.pth")
    ...
    def val(self, dataname, dataloader, evid):
        ...
        with tqdm(dataloader, unit="batch") as tepoch:
            for data in tepoch:
                ...
                ret = self.retrive_datarsna(idx, self.idxs)
                rtxt = ret["txt"]
                rimg =  ret["img"].to(frames.dtype).cuda()
                rl = ret["rl"].cuda()
                self.cnt += 1
    ...
    def setup_rag(self):
        ...
        with tqdm(self.public_dset_loader, unit="batch") as tepoch:
            ...
            for data in tepoch:
                ...
                gt = data["label"]
                with torch.no_grad():
                    if self.args.method == "racct" or self.args.method == "ragpt":
        ...
        self.mappingsval = self.generate_RAG_mapping(self.server.val_loader, self.img_vec, labels, self.server.model, self.round)
        self.mappingstest = self.generate_RAG_mapping(self.server.test_loader, self.img_vec, labels, self.server.model, self.round)
    ...
    def retrive_data(self, local_idxs, global_idxs):
        global_idx = [0]
        if self.round == 0:
            map_idx = operator.itemgetter(*local_idxs)(self.mappingstest)
