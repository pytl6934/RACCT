...
class FedAvgIn:
    def __init__(self, args, logger, wandb):
        ...
        self.ap = MultilabelAveragePrecision(num_labels=14).cuda()

        self.val_track = []

        self.disease_dict = {
            "Atelectasis": 0, "Cardiomegaly": 1, "Consolidation": 2, "Edema": 3,
            "Enlarged Cardiomediastinum": 4, "Fracture": 5, "Lung Lesion": 6, "Lung Opacity": 7,
            "No Finding": 8, "Pleural Effusion": 9, "Pleural Other": 10, "Pneumonia": 11,
            "Pneumothorax": 12, "Support Devices": 13
        }

    def train_distill_epoch(self, teacher_model, student_model) :
        """
        使用 mm_model 作为教师模型，img_model 作为学生模型进行知识蒸馏。
        :param teacher_model: mm_model (torch.nn.Module)
        :param student_model: img_model (torch.nn.Module)
        :param optimizer: img_model 的优化器
        :param criterion_bce: BCEWithLogitsLoss 损失函数
        :param grad_scaler: 混合精度训练使用的 GradScaler
        """
        ...
        with tqdm(self.server.val_loader, unit="batch") as tepoch:
            for data in tepoch:
                ...
                label = data["label"].float().cuda()  # BCEWithLogitsLoss 需要 float 类型
                text = data["text"]
                ...
                with torch.no_grad():
                    # 教师模型推理得到软标签
                    teacher_out = teacher_model(frames, text, "", self.modality)
                    teacher_logits = teacher_out["logits"].detach()

                # 学生模型前向传播
                student_out = student_model(frames, text, "", self.modality)
                student_logits = student_out["logits"]

                # 分类损失
                loss_task = criterion(student_logits, label)
                ...
                gradnorm = torch.nn.utils.clip_grad.clip_grad_norm_(student_model.parameters(), 2.0)
                grad_scaler.step(optimizer)
                grad_scaler.update()

                tmploss += loss.item()
                task_loss_total += loss_task.item()
                distill_loss_total += loss_distill.item()

                tepoch.set_postfix(
                    TaskLoss=loss_task.item(),
                    DisLoss=loss_distill.item()
                )

        avg_loss = tmploss / len(self.server.val_loader)
        avg_task_loss = task_loss_total / len(self.server.val_loader)
        avg_distill_loss = distill_loss_total / len(self.server.val_loader)

        print(f"Knowledge Distill Epoch Loss: {avg_loss:.4f} | Task: {avg_task_loss:.4f} | Distill: {avg_distill_loss:.4f}")
        return avg_loss
    ...
    def setup_clients(self):
        ...
        self.server = ServerClassificationTrainer(self.args, self.args.server_config_path, "MIMIC", self.logger, "noRAG",  False)
        ...
        # self.imgagg = [len(cl.train_set) for cl in img_clients]
        # imgsum = sum(self.imgagg)
        # self.imgagg = [1.0 * len(cl.train_set)/imgsum for cl in img_clients]

        # self.mmagg = [len(cl.train_set) for cl in mm_clients]
        # mmsum = sum(self.mmagg)
        # self.mmagg = [1.0 * len(cl.train_set)/mmsum for cl in mm_clients]

        # self.mm_model = copy.deepcopy(self.server.model)
        # self.mm_model.cpu()
        # self.img_model = copy.deepcopy(self.server.model)
        # self.img_model.cpu()            
    ...
    def test(self):
        ...
        with tqdm(self.server.test_loader, unit="batch") as tepoch:
            for data in tepoch:
                ...
                text = data["text"]
                with torch.no_grad():
                    # output = self.server.model(frames, text, label, "", self.txtproto, "testmm")
                    output = self.server.model(frames, text, "", "mm")
                    self.evaluator.update(output['logits'].sigmoid(), label.long())
                    preds = output['logits'].sigmoid()
                    self.ap.update(preds, label.long())

        metrics = self.evaluator.compute()
        # PRAUC = self.ap.compute().mean().item()
        print( f"round: {self.cur_comms},mm test AUC(Aggregrated): {metrics['AUC'].item():.5f}")
        self.logger.info(f"round: {self.cur_comms} ,mm Test AUC(Aggregrated): {metrics['AUC'].item():.5f}, test ACC {metrics['ACC'].item():.5f}, macroF1 {metrics['MultilabelF1Score'].item():.5f}")
    ...
    def aggregate(self):
        ...
        # 准备聚合字典
        global_dict = {k: torch.zeros_like(v, dtype=torch.float32).cpu() for k, v in server_state_dict.items()}

        # Step 1: 遍历每个客户端，计算其模型与服务器模型之间的 L1 差异
        for i, client in enumerate(self.clients):
            client_path = f"./CARckpts/Tmp/{self.args.prefix}_{i}.pth"
            client_state_dict = torch.load(client_path, map_location='cpu').state_dict()

            l1_norm_diff = 0.0
            for k in server_state_dict.keys():
                if any(substring in k for substring in ["num_batches_tracked", "embeddings.position_ids", "PersonalPrompt"]):
                    continue

                server_param = server_state_dict[k]
                client_param = client_state_dict.get(k)

                if client_param is not None:
                    diff = (client_param.float() - server_param.float()).abs()
                    l1_norm_diff += diff.sum().item()

            l1dis.append(l1_norm_diff)

        # Step 2: 归一化 l1dis
        l1dis_tensor = torch.tensor(l1dis)
        l1dis_normalized = l1dis_tensor / l1dis_tensor.sum()

        # Step 3: 新的 aggweight = 原 aggweight * l1dis_normalized，再归一化
        new_aggweight = self.aggweight * l1dis_normalized
        new_aggweight = new_aggweight / new_aggweight.sum()

        # Step 4: 使用新的 aggweight 进行加权平均聚合
        for i, client in enumerate(self.clients):
            client_path = f"./CARckpts/Tmp/{self.args.prefix}_{i}.pth"
            client_state_dict = torch.load(client_path, map_location='cpu').state_dict()
            ...
            for k in global_dict.keys():
                ...
                para = client_state_dict.get(k)
                if para is not None:
                    global_dict[k] += para.float() * new_aggweight[i]
        ...
        #         if para is not None:
        #             global_dict[k] += para.float() * self.aggweight[i] 
        # self.img_model.load_state_dict(global_dict) 

        # # self.train_distill_epoch(self.mm_model, self.img_model)

        # self.mm_model = self.mm_model.cpu()
        # self.img_model = self.img_model.cpu()
        # mm_state_dict = self.mm_model.state_dict()
        # img_state_dict = self.img_model.state_dict()

        # global_dict = {k: torch.zeros_like(v, dtype=torch.float32).cpu() for k, v in self.server.model.state_dict().items()}
        # for k in global_dict.keys():
        #     if any(substring in k for substring in ["num_batches_tracked", "embeddings.position_ids", "PersonalPrompt"]):
        #         continue
        #     para_mm = mm_state_dict.get(k)
        #     para_img = img_state_dict.get(k)
        #     if para_mm is not None and para_img is not None:
        #         aggregated_para = 0.8 * para_mm.float() + 0.2 * para_img.float()
        #         global_dict[k] += aggregated_para 
    ...
    def val(self):
        ...
        PRAUC = self.ap.compute().mean().item()
        metrics = self.evaluator.compute()
        print( f"round: {self.cur_comms},mm val AUC(Aggregrated): {metrics['AUC'].item():.5f}, PRAUC {PRAUC:.5f} ")
        self.logger.info(f"round: {self.cur_comms} , val AUC(Aggregrated): {metrics['AUC'].item():.5f}, val ACC {metrics['ACC'].item():.5f},  macroF1 {metrics['MultilabelF1Score'].item():.5f},  PRAUC {PRAUC:.5f}")
    ...
    def run(self):
        ...
        for comms in range(10000):
            ...
            # if comms < 40:
            #     for i in range(4):
            #         self.clients[i].run(comms, self.server.model, self.txtproto)
            #         sumloss += self.clients[i].loss
            # else:
            # for i in range(len(self.clients)):
            #     self.clients[i].run(comms, self.server.model, self.txtproto)
            #     sumloss += self.clients[i].loss
            # sumloss /= 1.0 * len(self.clients)
            
            for i in range(len(self.clients)):
                self.clients[i].run(comms, self.server.model, "")
                sumloss += self.clients[i].loss
            sumloss /= 1.0 * len(self.clients)
        
            self.logger.info(f"avgloss {sumloss:.4f}")
            print(f"---------------------avgloss {sumloss:.4f}--------------------")
            ...
            if cur_auc > self.val_auc:
                ...
                self.server.save_best(self.cur_comms, self.logger)
            self.cur_comms +=1
